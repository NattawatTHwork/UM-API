...
class Users
{
    ...
    public function getUserById($encryptedUserId)
    {
        ...
        $user['user_id'] = $this->encryption->encrypt($user['user_id']);
        $user['role_id'] = $this->encryption->encrypt($user['role_id']);
        return $user;
    }

    public function createUser($username, $userPassword, $firstname, $lastname, $encryptedRoleId)
    {
        ...
        $result = pg_execute($this->connection, "create_user", array($username, $userPassword, $firstname, $lastname, $RoleId));
        if (!$result) {
    ...
    public function updateUser($encryptedUserId, $firstname, $lastname, $encryptedRoleId, $statusflag)
    {
        $userId = $this->encryption->decrypt($encryptedUserId);
        $RoleId = $this->encryption->decrypt($encryptedRoleId);
        ...
        $result = pg_execute($this->connection, "update_user", array($userId, $firstname, $lastname, $RoleId, $statusflag));
        if (!$result) {
    ...
    {
        $query = 'SELECT user_id, username, user_password, tb_users.is_deleted, firstname, lastname, tb_users.statusflag, role 
        FROM cm_sap.tb_users INNER JOIN cm_sap.tb_roles ON tb_users.role_id = tb_roles.role_id WHERE username = $1';
